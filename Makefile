push:
	git checkout -b $(tag)
	jenkinsversion=$(dockerversion) dockerversion=$(jenkinsversion) dockercomposeversion=$(dockercomposeversion)  ./envsubstSc.sh < Dockerfile-$(platform)  2> /dev/null > Dockerfile
